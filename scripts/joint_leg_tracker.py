...
from people_msgs.msg import Person as PeoplePerson
...
class KalmanMultiTracker:
    ...
    def publish_tracked_people(self, now):
        """
        Publish markers of tracked people to Rviz, to <people_tracked> topic and <people> topic
        """
            ...
            for person in self.objects_tracked:
                if person.is_person == True:
                    if self.publish_occluded or person.seen_in_current_scan: # Only publish people who have been seen in current scan, unless we want to publish occluded people
                        ...
                        # publish to people_tracked and people topic
                        new_person = Person()
                        ...
                        new_people_person = PeoplePerson()
                        new_people_person.name = str(person.id_num)
                        new_people_person.position.x = ps.point.x
                        new_people_person.position.y = ps.point.y
                        new_people_person.velocity.x = person.vel_x
                        new_people_person.velocity.y = person.vel_y
                        people_msg.people.append(new_people_person)
